...
@router.get("/servers/{server_name}", summary="获取指定GPU服务器配置")
async def get_gpu_server(server_name: str) -> Dict[str, Any]:
    """获取指定GPU服务器的配置"""
    try:
        config = get_config()
        server = config.get_gpu_server(server_name)
        ...
        return {
            ...
            "message": f"获取服务器'{server_name}'配置成功"
        }
    except HTTPException:
        raise
    except Exception as e:
